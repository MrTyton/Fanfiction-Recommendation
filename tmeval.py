...
''' % (program_shortdesc, str(__date__))
    try:
        ...
        parser.add_argument("-t", "--author", dest="author", default=None, help="authorID to use for evaluation")
        ...
        if args.author is None:
            ...
            auth  =[int(args.author)]
        evalr = evaluator.Evaluator(oleg, authorIDs=auth, datadir=args.basedir, full=True, resultsdir="{}/results3".format(args.basedir)) # implements/overrides favorite_likelihood(self, storyID, favorites)
        evalr.evaluate()
        #(in parallel)...
